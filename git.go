...
func LogHostKeyCallback(log eblog.LogFunc) gossh.HostKeyCallback {
	return func(hostname string, remote net.Addr, key gossh.PublicKey) error {
		...
		if s := sb.String(); !strings.Contains(s, "fake-public-key ZmFrZSBwdWJsaWMga2V5") {
			log(eblog.LevelInfo, "#1: 🔑 Got host key: %s", strings.TrimSpace(s))
		}
		return nil
	}
}
...
func SetupRepoAuth(options *Options) transport.AuthMethod {
	if options.GitURL == "" {
		options.Logger(eblog.LevelInfo, "#1: ❔ No Git URL supplied!")
		return nil
	}
	if strings.HasPrefix(options.GitURL, "http://") || strings.HasPrefix(options.GitURL, "https://") {
		...
		if options.GitUsername == "" && options.GitPassword == "" {
			options.Logger(eblog.LevelInfo, "#1: 👤 Using no authentication!")
			return nil
		}
		// Basic Auth
		...
		options.Logger(eblog.LevelInfo, "#1: 🔒 Using HTTP basic authentication!")
	...
	options.Logger(eblog.LevelInfo, "#1: 🔑 Using SSH authentication!")

	var signer ssh.Signer
	if options.GitSSHPrivateKeyPath != "" {
		s, err := ReadPrivateKey(options.GitSSHPrivateKeyPath)
		if err != nil {
			options.Logger(eblog.LevelError, "#1: ❌ Failed to read private key from %s: %s", options.GitSSHPrivateKeyPath, err.Error())
		} else {
			options.Logger(eblog.LevelInfo, "#1: 🔑 Using %s key!", s.PublicKey().Type())
			signer = s
		}
	}
	...
	if signer == nil {
		options.Logger(eblog.LevelError, "#1: 🔑 No SSH key found, falling back to agent!")
		auth, err := gitssh.NewSSHAgentAuth(options.GitUsername)
		if err != nil {
			options.Logger(eblog.LevelError, "#1: ❌ Failed to connect to SSH agent: %s", err.Error())
			return nil // nothing else we can do
		}
		if os.Getenv("SSH_KNOWN_HOSTS") == "" {
			options.Logger(eblog.LevelWarn, "#1: 🔓 SSH_KNOWN_HOSTS not set, accepting all host keys!")
